...
// SCSS 컴파일 오류를 명확하게 표시하는 플러그인
function scssErrorHandler() {
  return {
    name: 'scss-error-handler',
    transform(code, id) {
      // SCSS 파일은 transform에서 처리하지 않음
      return null;
    },
    buildEnd(error) {
      if (error) {
    ...
    handleHotUpdate({ file, server }) {
      // 개발 환경에서 SCSS 파일 변경 시 처리
      if (file.endsWith('.scss')) {
        // SCSS 파일 변경은 Vite가 자동으로 처리
      }
    },
  };
}
...
export default defineCampaignConfig({
  ...
  plugins: [scssErrorHandler(), copyLibFilesAndTransformHtml()],
});
