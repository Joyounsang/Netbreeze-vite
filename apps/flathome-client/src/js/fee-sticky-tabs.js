...
import { getLenis, scrollTo as smoothScrollTo } from '@/js/smooth-scroll.js';
...
function createStickyTabGroup(tabBar) {
  ...
  function scrollToPanel(targetId) {
    ...
    if (getLenis()) {
      smoothScrollTo(top, { onComplete: finishClickScroll });
      scrollEndTimer = window.setTimeout(finishClickScroll, 1200);
      return;
    }
